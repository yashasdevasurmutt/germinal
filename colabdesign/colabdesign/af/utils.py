...
class _af_utils:  
  ...
  def save_pdb(self, filename=None, get_best=True, renum_pdb=True, aux=None, save_all=True):
    '''
    save pdb coordinates (if filename provided, otherwise return as string)
    - get_best: False to get the last sampled sequence
    - save_all: True to save all in same PDB
    '''
    if aux is None:
    ...
    p_str = ""
    p_shape = p["atom_positions"].shape[0] if save_all else 1 
    for n in range(p_shape):
