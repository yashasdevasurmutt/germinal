...
class _af_loss:
  ...
  def _loss_binder(self, inputs, outputs, aux):
    ...
    fw_id = zeros.at[-bL:].set(1)

    '''
    Decide if we want to use cdr-epitope distance instead of binder-epitope. 
    This is useful for antibody-antigen design but loss function is not purely distance-based
    since we also want to optimize the binder pose. default is False.
    '''
    if self.use_pos_distance and ("pos" in self.opt):
        pos_id = zeros.at[opt["pos"]].set(mask[opt["pos"]])
        fw_id = zeros.at[-bL:].set(1).at[opt["pos"]].set(0)
    else:
        pos_id = binder_id.copy()
        
    fw_i_con_loss = get_con_loss(inputs, outputs, opt["i_con"], mask_1d=target_id, mask_1b=fw_id)
    
    if "hotspot" in opt:
      target_id = zeros.at[opt["hotspot"]].set(mask[opt["hotspot"]])
      i_con_loss = get_con_loss(inputs, outputs, opt["i_con"], mask_1d=target_id, mask_1b=pos_id)
      if self.dimer:
        target_id_dimer = zeros.at[opt["hotspot_dimer"]].set(mask[opt["hotspot_dimer"]])
        i_con_loss_dimer = get_con_loss(inputs, outputs, opt["i_con"], mask_1d=target_id_dimer, mask_1b=pos_id)
        i_con_loss = jnp.minimum(i_con_loss, i_con_loss_dimer)
    else:
      i_con_loss = get_con_loss(inputs, outputs, opt["i_con"], mask_1d=pos_id, mask_1b=target_id)
      
    if self.opt["i_con"]["framework_contact_loss"]:
        i_con_loss = i_con_loss * (i_con_loss/jnp.maximum(fw_i_con_loss-self.opt['i_con']['framework_contact_offset'], 1e-8))

    # unsupervised losses
    aux["losses"].update({
      "i_plddt": get_plddt_loss(outputs, mask_1d=pos_id),
      ...
      # interface
      "i_con":   i_con_loss,
      "i_pae":   get_pae_loss(outputs, mask_1d=pos_id, mask_1b=target_id),
    })

    # supervised losses
    if self.binder_redesign:
      aln = get_rmsd_loss(inputs, outputs, L=tL, include_L=False)
      align_fn = aln["align"]
      
      # compute cce of binder + interface
      aatype = inputs["aatype"]
      if getattr(self, "binder_redesign", False) and ("pos" in opt):
        cce = get_dgram_loss(inputs, outputs, aatype=aatype, return_mtx=False, pos=self.opt["pos"], target_len=tL)
        partial_binder_redesign = True
      else:
        cce = get_dgram_loss(inputs, outputs, aatype=aatype, return_mtx=True)
        partial_binder_redesign = False
      ...
      aux["losses"].update({
        "rmsd":      aln["rmsd"],
        "dgram_cce": cce if partial_binder_redesign else (cce[-bL:].sum()  / (mask[-bL:].sum() + 1e-8)),
...
# loss functions
####################
def get_dgram_loss(inputs, outputs, copies=1, aatype=None, return_mtx=False, pos=None, target_len=None):
  ...
  if pos is not None:
    #Keep only dgram of binder framework
    tmp = inputs["seq_mask"].copy()
    tmp = tmp.at[pos].set(0).at[:target_len].set(0)
    weights = jnp.where(tmp,weights,0)
  else:
