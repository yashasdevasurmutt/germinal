  ...
  def _update_template(self, inputs, key):
    ...
    if "batch" in inputs:
      ...
      for k,v in template_feats.items():
        ...
        if partial_binder_redesign:
          tL = self._target_len
          if k in ["template_dgram"]:
            # Eliminate any dgram info between chains
            inputs[k] = inputs[k].at[0,tL:,:tL].set(0).at[0,:tL,tL:].set(0).at[0,pos[:,None],pos[None,:]].set(0)
          else:
            inputs[k] = inputs[k].at[0,pos].set(0)
...
def update_seq(seq, inputs, seq_1hot=None, seq_pssm=None, mlm=None):
  ...
  if seq_pssm is None: seq_pssm = seq # = hard
  target_feat = seq_1hot[0,:,:20] # = seq['pseudo']
  ...
  # msa feat has seq['pseudo'] from 0-22 and seq['hard'] from 25-48
  # target_feat has seq['pseudo']
